...
ENV \
    ...
    GEOSERVER_DATA_DIR_DATA=/opt/geoserver/data_dir/data \
...
RUN groupadd -r ${GROUP_NAME} -g ${GEOSERVER_GID} && \
    ...
    mkdir -p  ${GEOSERVER_DATA_DIR} ${GEOSERVER_DATA_DIR_DATA} ${CERT_DIR} ${FOOTPRINTS_DATA_DIR} ${FONTS_DIR} \
...
VOLUME ["${GEOSERVER_DATA_DIR}", "${GEOSERVER_DATA_DIR_DATA}", "${CERT_DIR}", "${FOOTPRINTS_DATA_DIR}", "${FONTS_DIR}"]
